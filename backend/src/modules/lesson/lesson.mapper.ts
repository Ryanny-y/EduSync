...
export const mapLessonToDto = (lesson: any): LessonDto => ({
  ...
  materials: lesson.materials?.map((m: any) => ({
    ...
    driveFileId: m.file.path,
    webViewLink: m.file.webViewLink || "",
    webContentLink: m.file.webContentLink || "",
    size: m.file.size,
  })) || [],
});
...
export const mapMimeTypeToFileType = (mimeType: string): FileType => {
  ...
  if (mimeType.includes("presentation") || mimeType.includes("powerpoint")) return FileType.PPT;
  if (mimeType.includes("word") || mimeType.includes("document")) return FileType.DOC;
